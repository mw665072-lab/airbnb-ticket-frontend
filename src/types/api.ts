...
export interface FlightOffer { offerId: string; airline: { code: string; name: string; logoUrl?: string }; totalAmount: number; currency: string; cabinClass: CabinClass; slices: FlightSlice[]; baggage?: { checkedBags?: number; cabinBags?: number; checkedKg?: number; cabinKg?: number }; refundable?: boolean; expiresAt?: string; paymentRequirements?: { requiresInstantPayment: boolean; priceGuaranteeExpiresAt?: string | null }; source?: "duffel"; }
export interface SearchFlightsPayload { tripType: TripType; segments: Array<{ origin: string; destination: string; departureDate: string }>; returnDate?: string; cabinClass: CabinClass; passengers: { adults: number; children: number; infants: number }; }
...
export interface CreateBookingRequestPayload { offerId: string; contact: { name: string; email: string; phone: string }; trip: { type: TripType; segments: SearchFlightsPayload["segments"]; returnDate?: string; cabinClass: CabinClass; passengers: SearchFlightsPayload["passengers"] }; passengers: PassengerPayload[]; notes?: string; }
...
export interface BookingRequestSummary { id: string; reference: string; route?: string; departureDate?: string; contact?: { name: string; email: string; phone?: string }; contactName?: string; paxCount?: number; status: BookingRequestStatus; quote?: Quote; unreadMessages?: number; createdAt: string; assignedAdmin?: User | null; }
export interface BookingRequestDetail extends BookingRequestSummary { trip?: CreateBookingRequestPayload["trip"]; passengers?: PassengerPayload[]; notes?: string; offerId?: string | null; statusHistory?: Array<{ status: BookingRequestStatus; note?: string; at: string }>; changeRequested?: boolean; messages?: Message[]; payment?: Payment; linkedPayment?: Payment | null; linkedBooking?: Booking | null; }
...
export interface Booking { id?: string; reference: string; pnr?: string; airline?: string; eTicketNumbers?: string[]; amount?: number; currency?: string; status: "confirmed" | "ticketed" | "cancelled" | "refunded"; contact?: { name: string; email: string; phone?: string }; route?: string; segments?: Array<Record<string, unknown>>; documents?: Array<{ name: string; url: string }>; payment?: Payment; createdAt?: string; }
export interface Customer { id: string; name: string; email: string; phone?: string; country?: string; stats?: { requests: number; bookings: number; totalSpent: number }; lastActivityAt?: string; createdAt?: string; timeline?: Array<{ type?: string; message?: string; createdAt?: string; [key: string]: unknown }>; requests?: BookingRequestSummary[]; bookings?: Booking[]; }
...
export interface DashboardData { today: { newRequests: number; confirmedBookings: number; revenue: number; currency: string }; counters: { pendingRequests: number; awaitingPayment: number; openEnquiries: number; disputes: number }; last30Days: { requests: number; bookings: number; conversionRate: number; revenue: number; profit: number }; chart: Array<{ date: string; requests: number; bookings: number; revenue: number }>; recentRequests: BookingRequestSummary[]; }
